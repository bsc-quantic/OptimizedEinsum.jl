...
function Makie.plot!(f::GridPosition, path::ContractionPath; colormap = to_colormap(:viridis)[begin:end-10], kwargs...)
    ...
    cbar = Colorbar(f[1,2], get_edge_plot(p), label=raw"\log_{2}(size)", flip_vertical_label=true, labelsize = 34)
    cbar.height = Relative(5/6)

    cbar2 = Colorbar(f[1,0], get_node_plot(p), label=raw"\log_{10}(flops)", flipaxis=false, labelsize = 34)
    cbar2.height = Relative(5/6)
